...
class BFSStrategy(IncalmoStrategy, ABC, name="bfs_strategy"):
    """
    A BFS (Breadth-First Search) graph traversal attack strategy.

    This strategy spreads across the network level-by-level, fully exploring
    all reachable hosts at the current depth before moving deeper. It proceeds
    through three phases:
      1. InitialAccess - scan and gather information from hosts that already
         have agents, then seed the attack-path queue.
      2. Spread - process the front of the queue (BFS order), escalate
         privileges, collect host information, exfiltrate any critical data,
         and append newly discovered paths to the back of the queue.
      3. Finished - all agents are done and the queue is empty; signal
         completion.
    """
    ...
    def __init__(self, config: AttackerConfig, **kwargs):
        ...
        self.state = BFSState.InitialAccess

        # Per-agent state tracking: agent paw -> AgentState
        self.agent_states: dict[str, AgentState] = {}

        # BFS queue of AttackPath objects to process
    ...
    async def act(self, world_model: dict) -> None:
        ...
        """
        if self.state == BFSState.InitialAccess:
            await self._initial_access(world_model)

        elif self.state == BFSState.Spread:
            await self._spread()

        elif self.state == BFSState.Finished:
            self.logger.info("BFS campaign finished.")
    ...
    async def _initial_access(self, world_model: dict) -> None:
        ...
        then seed the BFS queue with all reachable attack paths.
        """
        ...
        # Seed the BFS queue from every initial host
        paths = []
        for host in world_model["hosts_with_agents"]:
        ...
        # BFS: append to the back of the queue
        self.attack_path_queue.extend(paths)
        self.logger.info(f"Seeded BFS queue with {len(paths)} attack paths")

        self.state = BFSState.Spread

    async def _spread(self) -> None:
        """
        One BFS iteration:
          - If queue is empty and all agents are done, transition to Finished.
          - Otherwise pop the front path, attempt a lateral move, then process
            every agent that is in the FIND_HOST_INFORMATION state.
        """
        if self._all_agents_finished() and len(self.attack_path_queue) == 0:
            self.state = BFSState.Finished
            return

        # Pop from the front for BFS ordering
        ...
        for agent_paw, agent_state in self.agent_states.items():
            ...
            # BFS: append new paths to the back of the queue
            new_paths = self.attack_graph_service.get_possible_targets_from_host(host)
            random.shuffle(new_paths)
            self.attack_path_queue.extend(new_paths)
            self.logger.info(f"Added {len(new_paths)} new paths to BFS queue")
    ...
    async def step(self) -> bool:
        ...
        Returns:
            bool: True when all hosts reachable via BFS have been processed,
        ...
        if self.state == BFSState.Finished:
            self.logger.info("BFS strategy completed successfully")
            return True
